...
DEFAULT_INPUT_COLUMN_CANDIDATES = [
    "Input",          # exact match wins before any substring check
    "Singlish Input",
    "Test Input",
    "Source",
    "Sentence",
    "Text",
    "Singlish",       # kept last to avoid matching 'Singlish input types covered'
]
...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                # ── page-alive check: re-navigate if browser closed the tab ──
                try:
                    page.title()
                except Exception:
                    print("  [warn] Page closed – re-navigating...")
                    page = browser.new_page()
                    page.set_default_timeout(max(1000, int(args.timeout_ms)))
                    page.goto(args.url, wait_until="domcontentloaded")
                    try:
                ...
                # ── clear output textarea via JS so stale text never blocks us ──
                try:
                ...
                # ── wait for non-empty output (no stale-check needed after clear) ──
                actual_output = ""
